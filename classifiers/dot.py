...
def train_classifier(clf, model, classifier, params=None):
    if params is None:
        ...
        best_clf = grd.best_params_
        best_score = best_clf.score()
        test_report(best_clf, Xtest, ytest)
...
scores[0, 2] = train_classifier(svc, "vgg16", "xgb", parameters)

rf = RandomForestClassifier()
param_grid = {'bootstrap': [True], 'max_depth': [80, 90, 100, 110], 'max_features': [2, 3], 'min_samples_leaf': [3, 4, 5], 'min_samples_split': [8, 10, 12], 'n_estimators': [100, 200, 300, 1000]
}
scores[0, 3] = train_classifier(svc, "vgg16", "xgb", parameters)